...
class TestPinStorage(unittest.TestCase):
    def setUp(self):
        ...
        store.add_student(self.conn, "40217", "Block 3")

    def test_set_and_check(self):
        store.set_pin(self.conn, "40217", "1234")
        self.assertTrue(store.check_pin(self.conn, "40217", "1234"))
        self.assertFalse(store.check_pin(self.conn, "40217", "9999"))
    ...
    def test_has_pin(self):
        ...
        store.set_pin(self.conn, "40217", "1234")
        self.assertTrue(store.has_pin(self.conn, "40217"))

    def test_a_reset_clears_it(self):
        store.set_pin(self.conn, "40217", "1234")
        store.clear_pin(self.conn, "40217")
        self.assertFalse(store.has_pin(self.conn, "40217"))
        self.assertFalse(store.check_pin(self.conn, "40217", "1234"))

    def test_setting_again_replaces(self):
        store.set_pin(self.conn, "40217", "1234")
        store.set_pin(self.conn, "40217", "5678")
        self.assertFalse(store.check_pin(self.conn, "40217", "1234"))
        self.assertTrue(store.check_pin(self.conn, "40217", "5678"))

    def test_an_unknown_student_never_matches(self):
        self.assertFalse(store.check_pin(self.conn, "49999", "1234"))
...
class TestSignInFlow(unittest.TestCase):
    def setUp(self):
        ...
        store.add_student(self.conn, "40217", "Block 3")
        store.add_student(self.conn, "40218", "Block 3")
    ...
    def test_confirming_an_unknown_number_carries_on_rather_than_looping(self):
        ...
        r = self.client.post("/signin", data={"step": "id", "student_id": "49999",
    ...
    def test_mistyped_confirmation_is_caught(self):
        self.enter_id("40217")
        r = self.enter_pin("40217", "1234", "4321")
    ...
    def test_a_teacher_reset_lets_them_choose_again(self):
        store.set_pin(self.conn, "40217", "1234")
        self.client.post("/teacher/pin/reset", data={"student_id": "40217"})
        self.assertIn("Choose a PIN", self.enter_id("40217").data.decode())
...
class TestIdentityComesFromTheSession(TestSignInFlow):
    ...
    def test_practice_is_recorded_against_the_signed_in_student(self):
        self.sign_in("40217")
        store.record_event(self.conn, "40217", "vocab:x:la_en", None, "x", "right")
        self.assertEqual(len(store.events_for_student(self.conn, "40217")), 1)
        self.assertEqual(len(store.events_for_student(self.conn, "40218")), 0)

    def test_a_student_id_in_the_url_is_ignored(self):
        self.sign_in("40217")
        r = self.client.get("/drill/session?student=40218&week=all&direction=both")
        self.assertEqual(r.status_code, 200)
        self.assertIn("40217", r.data.decode())
        self.assertNotIn("40218", r.data.decode())

    def test_progress_shows_your_own(self):
        self.sign_in("40217")
        body = self.client.get("/drill/progress?student=40218").data.decode()
        self.assertIn("40217", body)
        self.assertNotIn("40218", body)
